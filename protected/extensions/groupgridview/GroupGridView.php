...
class GroupGridView extends CGridView
{
    ...
    public $subFooterExcludedColumns = array();
    ...
    private function renderTotalRow($row, $totals)
    {
        ...
        echo '<tr class="total">';
        foreach ($this->columns as $column) {
            if ($column->footer && !in_array($column->name, $this->subFooterExcludedColumns)) {
                if (in_array($column->name, $this->subFooterColumns)) {
                    ...
                    echo '<td class="extrarow" style="' . (isset($column->footerHtmlOptions['style']) ? $column->footerHtmlOptions['style'] : '') . '">' . $column->footer . '</td>';
                }
            } else {
